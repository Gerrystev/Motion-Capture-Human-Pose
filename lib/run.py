    ...
    for action in dataset[subject].keys():
        ...
        for cam_idx in range(len(keypoints[subject][action])):
            ...
            try:
                ...
                assert keypoints[subject][action][cam_idx][0].shape[0] >= mocap_length
                if keypoints[subject][action][cam_idx][0].shape[0] > mocap_length:
                    # Shorten sequence
                    keypoints[subject][action][cam_idx] = keypoints[subject][action][cam_idx][0][:mocap_length]
    ...
    for action in keypoints[subject]:
        for cam_idx, kps in enumerate(keypoints[subject][action]):
            ...
            try:
                ...
                kps = kps[0]
                kps[..., :2] = normalize_screen_coordinates(kps[0][..., :2], w=cam['res_w'], h=cam['res_h'])
